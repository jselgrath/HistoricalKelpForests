...
d1<-read_csv("./results/all_var_location3_all_yr.csv")%>% # this version of data only has years with kelp maps
  dplyr::select(year,location3,kelp_maxarea_p,enso1_5,yr_after_enso1_5,otter_u)%>%
...
fxn1<-formula(kelp_maxarea_p ~ year+enso1_5+yr_after_enso1_5+Location+otter_u+otter_period) #was otter abundance u
m1<-lm(fxn1, data=d2)
summary(m1)
# plot(m1)
