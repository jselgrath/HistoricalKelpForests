...
d1<-read_csv("./results/alldata_kelpmap_location3.csv")%>% # this version of data only has years with kelp maps
  dplyr::select(year,location3,kelp_area_p,enso,otter_abundance_u2,otter_b)%>%
...
# merge santa cruz because of limits to urchin data
# d1$location3[d1$location3=="Santa Cruz Bay"]<-"Santa Cruz"
# d1$location3[d1$location3=="Santa Cruz Outer"]<-"Santa Cruz"

#remove spaces for model
# d1$source2=d1$kelp_source
# d1$source2<-gsub(", ","",d1$source2)
# d1$source2<-gsub(" ","",d1$source2)

# d1$Location<-d1$location
# d1$Location<-gsub(" ","",d1$Location)

d1$Location<-d1$location%>%fct_relevel("Santa Cruz","Monterey Peninsula","Big Sur")

# combine santa cruz locations
# d2<-d1%>%
#   group_by(year,location3,location,Location,source2,kelp_source)%>%
#   summarize(kelp_area_relative=mean(kelp_area_relative),
#             kelp_area_p=mean(kelp_area_p),
#             enso_max_nq=mean(enso_max_nq),
#             otter_abundance_u=mean(otter_abundance_u),
#             otter_b=max(otter_b))



source("./bin/deets.R")
ggplot(d1,aes(x=year,y=kelp_area_p,color=Location))+geom_point()+deets3+facet_grid()
...
fxn1<-formula(kelp_area_p ~ year+enso+location+otter_period) #was otter abundance u
m1<-lm(fxn1, data=d2)
summary(m1)
# plot(m1)
...
m3g<-gls(fxn1,correlation=corAR1(form=~1|location3/year), ## was Location
        method="REML", data=d2)
summary(m3g) 
r3g<-resid(m3g, type = "normalized")
...
anova(m4,m5,m6) # m5 & m6 not different
