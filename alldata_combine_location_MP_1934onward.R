...
d1<-read_csv("./results/kelp_allsources_abundance_location.csv")%>%
  ...
  filter(year>1910,location=="Monterey Peninsula")%>%
...
d9<-d7%>%
  ...
  arrange(year)%>%
  glimpse()
write_csv(d9,"./results/all_var_location_2023.csv")
...
write_csv(d10,"./doc/sampsz_all_data_location_noPresOnly.csv")
...
write_csv(d20,"./results/all_var_location_2023_interpol.csv")
